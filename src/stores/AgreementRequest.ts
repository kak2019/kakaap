  ...
  constructor() {
   ...
   public get ValidDateToAsDate() {
    ...
    public get approvalLevel() {
      let approvalLvl = 'Agreement Controller';
      if (this.DecisionType === 'Non Sourcing Decision') {
        switch (this.AgreementType) {
          case 'Development':
          case 'Misc':
          case 'Material Supply logistic = VLMC':
          case 'Tooling':
          ...
          case 'Confidentiality': {
            if (this.IsDeviation === true) {
              approvalLvl = 'Manager';
            }
            break;
          }
          case 'Framework':
            approvalLvl = 'Vice President';
            break;
          case 'Price':
          case 'Raw Material': {
            switch (this.YearlySpend) {
              ...
              case '< 10 000 000 SVP':
                ...
                break;
              case '>= 10 000 000 EVP':
                approvalLvl = 'Executive Vice President';
                break;
              default: approvalLvl = 'Manager';
            }
            break;
          }
          default: approvalLvl = 'Agreement Controller';
        }
      }
      else {
        approvalLvl = 'Agreement Controller';
      }
        ...
        .then(buyerProperties => {
          ...
          const isUserAtGivenLevel = (user: any, managerLevel: string) => {
            const properties = user.UserProfileProperties.results;
              ...
              if (p.Key === "VolvoManagerLevel") {
                return p.Value === managerLevel;
              }
            }
          ...
          Promise.all(hierarchyPropsPromises)
            .then(managersProps => {
              const approvers = new Array(4);// Array.from({length: 4});//TODO-fix array.from
              approvers[0] = findManagerAtGivenLevel(managersProps, "N-4");
              approvers[1] = findManagerAtGivenLevel(managersProps, "N-3");
              approvers[2] = findManagerAtGivenLevel(managersProps, "N-2");
              approvers[3] = findManagerAtGivenLevel(managersProps, "N-1");
